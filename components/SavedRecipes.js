...
export default class SavedRecipes extends Component {
    ...
    componentDidMount() {
        ...
        .then(user => {
            console.log(user.users.recipes)
            let rawRecipe = user.users.recipes
            ...
            this.setState({
                userSavedRecipes: finalRecipeArray
            })
        })
        .catch(error => {
            console.log(error)
    ...
    render(){
        ...
        // let userSavedRecipes = this.state.userSavedRecipes
        return(
            <ScrollView>
                <Text>High</Text>
            {/* {
            {/* this.state.userSavedRecipes.map((current, index) => {
                let url = current.url
                return (
                    <Card
                    title={current.title}>
